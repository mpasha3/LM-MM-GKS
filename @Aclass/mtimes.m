...
if size(x,2)==1
    vec_flag=1;
    if strcmp(bc,'none') && A.transpose==1
     x=reshape(x,A.n-2*A.radius,A.m-2*A.radius);
    else 
    x=reshape(x,A.n,A.m);
    end
end
...
if strcmp(bc,'none')
    if A.transpose==0
        y=real(ifft2(eigA.*fft2(x)));
        y=y(r+1:end-r,r+1:end-r);
    else
        x=padarray(x,[r,r],0,'both');
        y=real(ifft2(eigA.*fft2(x)));
    end
else
    ...
    switch bc
        case 'periodic'
            x=padarray(x,[r,r],'circular','both');
        case 'reflexive'
            x=padarray(x,[r,r],'symmetric','both');
        case 'antireflexive'
            x=padarray(x,[r,r],'antisymmetric','both');
        case 'zero'
            x=padarray(x,[r,r],0,'both');
        otherwise
            error('Unknown bc')
...
%   When padding using the 'circular', 'replicate', 'symmetric', or
%   'antisymmetric' methods, A can be of any class.  B is of the same
%   class as A.
%
%   Example
%   -------
...
%   other.
%
%   Antisymmetric pad added 2006/02/23 by M. Donatelli
